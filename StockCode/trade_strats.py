...
def plot_from_strat(buy_days, sell_days, buy_prices, sell_prices, stocks, stocks_traded_idx):
    plot_random = random.sample(stocks_traded_idx, 5) if len(stocks_traded_idx) > 5 else np.arange(
        len(stocks_traded_idx))
...
### Excellent Long Strategy for Current Market Conditions.
...
def swing_(stocks, day_trade=False, hard_stop=True, model_type='ANN', model=False):
    stocks_traded_idx = []
    stats = []
    tickers = []
    stop_loss_percent = 0.8
    stop_loss_percent2 = 0.9
    take_gain_percent = 1.2
    buy_days = []
    sell_days = []
    buy_prices = []
    sell_prices = []
        ...
        while day < -2:
            if model_type == '2DCNN':
                prd = model.predict_classes(
                    np.expand_dims(np.expand_dims(stock.trade_data_tensor[day, :, :], axis=2),
                        axis=0))
            ...
            if all(condition):
                ...
                if stock.metrics[day, 2] < stop_loss:
...
# def swing_with_stop(stocks, day_trade=False):
#     stocks_traded_idx = []
#     stats = []
#     tickers = []
#     stop_loss_percent = 0.8
...
#                 buy_day.append(look_back + day)
#                 buy_price.append(stock.metrics[day, 0])
#                 if stock.metrics[day, 0] >= buy_price[-1] * take_gain_percent:
#                     sell_price.append(take_gain_percent * buy_price[-1])
#                     sell_day.append(look_back + day)
#                 elif stock.metrics[day, 2] <= stop_loss_percent * buy_price[-1]:
...
#                 else:
#                     current_high = np.max([stock.metrics[day, 0], stock.metrics[day, 3]])
#                     stop_loss = stop_loss_percent2 * current_high
#                     while True:
#                         day += 1
#                         if not day < -1:
#                             break
...
#                         if open > current_high:
#                             current_high = open
#                             stop_loss = current_high * stop_loss_percent2
#                         if close > current_high:
#                             current_high = close
#                             stop_loss = current_high * stop_loss_percent2
#             else:
